    ...
    <div className="flex shrink-0 gap-2 pr-2">
      {summaryTags.map((tag) => (
        <span key={tag.label} className="neo-pressed flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-1.5">
          <span className="text-[color:var(--md-primary)]"><Icon name={tag.iconKey} size={11} /></span>
          <span className="whitespace-nowrap text-[10px] font-medium text-[color:var(--md-on-surface-variant)]">{tag.label}</span>
        </span>
      ))}
    </div>
  );
